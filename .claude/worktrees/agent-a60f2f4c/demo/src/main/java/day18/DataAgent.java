...
public class DataAgent implements Agent {
    ...
    public DataAgent(String apiKey, boolean verbose, RagAgent ragAgent) {
    ...
    public DataAgent(String apiKey, RagAgent ragAgent) {
        this(apiKey, true, ragAgent);
    }
    ...
    public AgentMessage handle(AgentMessage message) {
        ...
        String content = "";
        if (message.getContent().contains("绩效")) {
            content = ragAgent.safeHandle(message.forward(ragAgent.getName(), message.getContent())).getContent();
        }

        if (queriedData.isEmpty() && content.isEmpty()) {
