...
public class PlanAndSolveOrchestrator implements Agent{
    ...
    private CompletableFuture<Map<String, AgentMessage>> solveAsync(String userRequest, List<TaskNode> plan) {
        ...
        return CompletableFuture.allOf(futureMap.values()
    ...
    public AgentMessage handle(AgentMessage message) {
        ...
        Map<String, AgentMessage> results = solveAsync(userRequest, plan).get();
    ...
    public CompletableFuture<AgentMessage> handleAsync(AgentMessage message) {
        String userRequest = message.getContent();
        return CompletableFuture
                .supplyAsync(() -> plan(userRequest), executor)   // Plan 异步执行
                .thenCompose(plan -> {                             // ← 你来填
                    // plan 为空时直接返回 failedFuture
                    // 否则调 solve() 拿到 results，再调 aggregateResults()
                    // 最终 return CompletableFuture.completedFuture(message.reply(...))
                    if (plan.isEmpty()) {
                        return CompletableFuture
                                .failedFuture(new RuntimeException("无法制定执行计划，请重新描述需求。"));
                    }
                    System.out.println("[Orchestrator] 计划节点数：" + plan.size());
                    return solveAsync(userRequest, plan).thenApply(
                            results -> {
                                String finalResponse = aggregateResults(userRequest, plan, results);
                                return message.reply(AgentMessage.MessageType.FINAL_RESPONSE, finalResponse);
                            }
